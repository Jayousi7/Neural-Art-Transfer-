...
def clac_mean_std(feat,eps=1e-5):
    size = feat.size()
    assert (len(size) == 4)
    N,C = size[:2]
...
def adain(content_feat,style_feat,alpha=1.0):
    ...
    style_mean,style_std = clac_mean_std(style_feat)
    content_mean,content_std = clac_mean_std(content_feat)
...
def calc_style_loss(gen_feats,style_fets):
    style_loss = torch.tensor(0.0, dtype=torch.float32,device=gen_feats[0].device)

    for gen_feat,style_feat in zip(gen_feats,style_fets):
        gen_mean,gen_std = clac_mean_std(gen_feat)
        style_mean,style_std = clac_mean_std(style_feat)
